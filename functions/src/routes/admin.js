...
router.post("/test-profile", asyncRoute(async (req, res) => {
  ...
  // This must be a "link" code, not a "view" one - the whole point of the
  // Admin test profile is manually canary-testing the real customer flow,
  // including adding to the tab, which a view-only code can never do
  // (resolveEffectiveUid() only ever resolves a claim to its owner's uid for
  // a "link" code - see middleware.js's linkedTargetFromClaim()). A "view"
  // code here used to leave the Add to basket button entirely missing, or
  // - worse - present but silently acting on the wrong (browsing device's
  // own) tab, depending on that device's unrelated prior history.
  let linkCode = targetSnap.exists ? targetSnap.data().adminViewCode : null;
  if (!targetSnap.exists) {
    ...
    for (let attempt = 0; attempt < 6 && !linkCode; attempt++) {
      const candidate = randomCode(10);
      const clash = await db().collection("codes").doc(candidate).get();
      if (!clash.exists) linkCode = candidate;
    }
    if (!linkCode) throw bad("Could not create a private Admin profile code.");
    ...
    batch.set(targetRef, {
      ...
      clonedFrom: sourceUserId, linkedUids: [], adminViewCode: linkCode,
    ...
    batch.set(db().collection("codes").doc(linkCode), {
      code: linkCode, userId: targetUserId, type: "link", active: true,
  ...
  if (linkCode) {
    const codeSnap = await db().collection("codes").doc(linkCode).get();
    codeIsValid = codeSnap.exists && codeSnap.data().active !== false
      && codeSnap.data().type === "link" && codeSnap.data().userId === targetUserId;
  }
  if (!codeIsValid) {
    linkCode = null;
    for (let attempt = 0; attempt < 6; attempt++) {
      const candidate = randomCode(10);
      const clash = await db().collection("codes").doc(candidate).get();
      if (!clash.exists) { linkCode = candidate; break; }
    }
    if (!linkCode) throw bad("Could not repair the Admin test profile link.");
    const now = FieldValue.serverTimestamp();
    const batch = db().batch();
    batch.set(targetRef, { adminViewCode: linkCode, updatedAt: now }, { merge: true });
    batch.set(db().collection("codes").doc(linkCode), {
      code: linkCode, userId: targetUserId, type: "link", active: true,
      purpose: "admin-test-profile", createdBy: req.uid, createdAt: now,
    });
  ...
  // No claims doc is pre-seeded here - the browser that actually opens this
  // URL calls the normal POST /store/link/accept flow itself (same as any
  // real multi-device invite), which is what correctly establishes
  // linkedUids membership and the claim, for whichever uid that browser
  // turns out to be signed in as.
  res.json({ url: `index.html?link=${encodeURIComponent(linkCode)}&profile=admin-test` });
}));
